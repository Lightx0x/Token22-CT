...
fn test_initialize() {
    ...
    let instruction = Instruction::new_with_bytes(
        program_id,
        &token22_ct::instruction::Initialize {}.data(),
        token22_ct::accounts::Initialize {
            payer: payer.pubkey(),
            counter,
    ...
    let counter_state = token22_ct::state::Counter::try_deserialize(&mut data).unwrap();
    assert_eq!(counter_state.count, 0);
    assert_eq!(counter_state.authority, payer.pubkey());

    let instruction = Instruction::new_with_bytes(
        program_id,
        &token22_ct::instruction::Increment {}.data(),
        token22_ct::accounts::Increment {
            counter,
